...
# Function to regularize the target signal based on the Fourier Transform
def regularize_target(predictor_signal, target_signal, max_bandwidth):
    # Compute the Fourier Transforms of predictor and target signals
    freq_xt, spectrum_xt = compute_fft(predictor_signal)
    freq_yt, spectrum_yt = compute_fft(target_signal)
    
    # Select the frequency components of the target signal that are below the max bandwidth
    valid_frequencies = np.abs(freq_yt) <= max_bandwidth
    truncated_spectrum_yt = spectrum_yt * valid_frequencies
    
    # Construct the regularized target signal by applying the Inverse Fourier Transform
    regularized_target = compute_ifft(truncated_spectrum_yt)
...
def process_signals(predictor_csv, target_csv, output_csv=None):
    ...
    # Step 2: Regularize each target variable based on max bandwidth and store results
    regularized_signals = {}
    for column in target_data.columns:
        target_signal = target_data[column].values
        regularized_target_signal = regularize_target(combined_predictor_signal, target_signal, max_bandwidth_predictor)
...
if __name__ == "__main__":
    ...
    output_csv = r"C:\Users\aryan\OneDrive\Desktop\Reservoir Classification\regularized_fft_target_signals.csv"  # Modify with actual path
